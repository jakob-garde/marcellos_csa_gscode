...
func main() {
	...
	// sheets API:
	{
		...
		//spreadsheetID := "15fK71g_KNd52QEZwrJ2i9MKsJSQrZ4azhDiHRrnkl0s"
		spreadsheetID := "1C9PIXa_Tm1eP0lHVF3073Nvo3NNYnerdqgqVLQmmMUw"
		readRange := "A:D"
		...
		// ???: how do I get the darn "array" of sheets?
		...
		// prints all sheet titles
		for _, s := range ss.Sheets {
			fmt.Printf("%s (%s)\n", s.Properties.Title, s.Properties.SheetType)
			...
			vals, err := svc.Spreadsheets.Values.Get(spreadsheetID, s.Properties.Title).Do()
			if err != nil {
				fmt.Println("could not get sheet values")
			}

			//fmt.Println(s.Properties.Title, vals)
		...
		_, err = svc.Spreadsheets.Values.Update(spreadsheetID, writeRange, body).
